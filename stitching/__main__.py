...
@click.option("--image-size", default=100)
...
def __stitch(filename, output, image_size, montage_size):
    ...
    for channel in range(channel_count):
        for chunk in range(n_chunks):
            ...
            images = [__pad(image, image_size) for image in images]
...
def __pad(image, image_size):
    ...
    if pad_x < 0:
        pad_x = float(image.shape[0] - image_size)
    if pad_y < 0:
        pad_y = float(image.shape[1] - image_size)
    ...
    temp_image = numpy.pad(image, (pad_width_x, pad_width_y), normal)
    return temp_image[(temp_image.shape[0]-image_size)/2: (temp_image.shape[0]+image_size)/2,(temp_image.shape[0]-image_size)/2:(temp_image.shape[0]+image_size)/2]
